...
(() => {
  ...
  function bindVisuals(){addHeaderAndFooter();addStyles();paintSongs();syncPlayerHeight();window.addEventListener("resize",syncPlayerHeight);const list=$("#song-list");if(list)new MutationObserver(()=>{paintSongs();}).observe(list,{childList:true,subtree:true});}
  ...
  function startGame(event){event.preventDefault();event.stopPropagation();const currentGame=createGame();currentGame.panel.hidden=false;setToolbarHidden(true);health=24;seconds=80;active=true;currentGame.status.textContent=`Health: ${health}`;for(let i=0;i<9;i++){const hole=currentGame.board.children[i];if(hole){hole.dataset.active="false";hole.textContent="🕳️";}}gameTimer=setInterval(()=>{if(--seconds<=0){finish(true);return;}spawnMole();},1000);spawnMole();}
