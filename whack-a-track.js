...
(() => {
  ...
  function addHeaderAndFooter() {
    ...
    if (!$("#rizney-site-adjustments")) {
      const style = document.createElement("style"); style.id = "rizney-site-adjustments"; style.textContent = `
        ...
        .top-area .donate,.top-area .context-link{top:16px!important;z-index:2;border:1px solid var(--gold);border-radius:999px;padding:5px 9px;color:var(--bright-gold);background:#160c1a;font:inherit;text-decoration:none}
        ...
        @media(max-width:500px){.top-area{min-height:78px!important;padding:12px 8px 14px!important}.top-area .donate,.top-area .context-link{top:14px!important}.top-area .donate{left:8px!important}.top-area .context-link{left:50%!important}.rizney-footer img{width:120px;max-height:90px}}
  ...
  function addStyles() {
    ...
    const style = document.createElement("style"); style.id = "rizney-animal-styles"; style.textContent = `
      ...
      #song-list .song .play{grid-column:3;grid-row:1;align-self:stretch;justify-self:end;width:52px;height:52px;min-height:52px;padding:3px;display:grid;place-items:center;overflow:hidden;background:transparent;border:none}
      #song-list .song .play img{display:block;width:100%;height:100%;object-fit:contain;pointer-events:none}
      #cards .card{background:#000}#cards .card .symbol{height:96px;display:grid;place-items:center;font-size:0}#cards .card .symbol img{width:96px;height:96px;object-fit:contain;display:block}#cards .card a{display:inline-block;margin-top:8px}
  ...
  function paintSongs(){document.querySelectorAll("#song-list .song").forEach((row,index)=>{const button=row.querySelector("button.play");if(!button||button.dataset.animalPainted==="true")return;const img=document.createElement("img");img.src=imageFor(index);img.alt=labelFor(animal(index));img.loading="lazy";button.innerHTML="";button.appendChild(img);button.dataset.animalPainted="true";});}
  function paintCards(){const cards=$("#cards");if(!cards)return;cards.querySelectorAll(".card").forEach((card,position)=>{const link=card.querySelector("a");if(!link)return;const symbol=card.querySelector(".symbol");if(!symbol)return;const img=document.createElement("img");img.src=imageFor(position);img.alt=labelFor(animal(position));img.loading="lazy";symbol.innerHTML="";symbol.appendChild(img);});}
  ...
  function createGame(){if(game)return game;const panel=document.createElement("section");panel.id="whack-a-track-game";panel.hidden=true;panel.innerHTML=`<h2>Whack-a-Track</h2><p id="wat-status">Whack the track before it escapes.</p><div id="wat-board"></div>`;const board=document.createElement("div");board.className="wat-board";panel.appendChild(board);const status=panel.querySelector("#wat-status");return {panel,board,status};}
  function hideMoles(){game?.board.querySelectorAll(".wat-hole").forEach(h=>{h.dataset.active="false";h.textContent="🕳️"})}
  function spawnMole(){if(!active||!game)return;const hole=[...game.board.children][Math.floor(Math.random()*game.board.children.length)];hideMoles();hole.dataset.active="true";hole.textContent="🐾";moleTimer=setTimeout(()=>{if(active&&hole.dataset.active==="true"){hole.dataset.active="false";hole.textContent="🕳️";health=Math.max(0,health-1);game.status.textContent=`Health: ${health}`;if(health<=0){finish(false);}}},1200);}
  ...
  function startGame(event){event.preventDefault();event.stopPropagation();const currentGame=createGame();game=currentGame;currentGame.panel.hidden=false;setToolbarHidden(true);health=24;seconds=80;active=true;currentGame.status.textContent=`Health: ${health}`;for(let i=0;i<9;i++){const hole=document.createElement("button");hole.type="button";hole.className="wat-hole";hole.textContent="🕳️";hole.addEventListener("click",()=>{if(!active||hole.dataset.active!=="true")return;hole.dataset.active="false";hole.textContent="✨";health=Math.min(24,health+1);currentGame.status.textContent=`Health: ${health}`;});currentGame.board.appendChild(hole);}gameTimer=setInterval(()=>{seconds-=1;if(seconds<=0){finish(true);return;}if(active){spawnMole();}},1000);spawnMole();}
